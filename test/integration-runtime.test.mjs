...
test('real PostgreSQL and Redis runtime contract', async (t) => {
  ...
  await t.test('expired job leases are recoverable and stale workers are fenced', async () => {
    ...
    assert.equal(
      await repository.renewJobLease(created.job.id, tokenA, {}, 60),
      null,
    );
    await assert.rejects(
  ...
  await t.test('outbox acknowledgements are fenced to the active claimant', async () => {
    ...
    const claimedA = await repository.claimOutbox('delivery-a', 'lock-a', 10);
    ...
    const claimedB = await repository.claimOutbox('delivery-b', 'lock-b', 10);
    assert.ok(claimedB.some((event) => event.id === eventId));
    assert.equal(
      await repository.markOutboxDelivered(eventId, 'delivery-a', 'lock-a'),
      false,
    );
    assert.equal(
      await repository.markOutboxDelivered(eventId, 'delivery-b', 'lock-b'),
      true,
    );
  });
